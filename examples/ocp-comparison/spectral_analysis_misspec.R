...
T_end  <- 2.0
t_sim  <- seq(0, T_end, length.out = 200)  # 200 pts, fs ≈ 100 Hz, Nyquist ≈ 50 Hz
dt_sim <- diff(t_sim)[1]
t_obs  <- t_sim[seq(1, length(t_sim), by = 4)]  # coarser observation grid
...
# 7a. Spectral dynamics: relative amplitude at each forcing frequency vs iteration
