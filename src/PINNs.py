...
class TrainingFrame:
    ...
    ic_weight: float = 0.0             # penalty weight active at this snapshot
...
class TrainingConfig:
    ...
    verbose: bool = True
    # NTK-based adaptive penalty weighting (Wang et al. 2021)
    adaptive_weights: bool = False
    ntk_update_freq: int = 1_000      # iterations between weight updates
    ntk_n_samples: int = 100          # collocation points used to estimate traces
    ntk_ema: float = 0.9              # EMA factor: w ← ema*w + (1-ema)*w_new
...
class PINN(nn.Module):
    ...
    def __init__(self, config: PINNConfig) -> None:
        ...
        if config.fourier_freqs is not None:
            self.embedding: Optional[FourierEmbedding] = FourierEmbedding(config.fourier_freqs)
            in_dim = self.embedding.out_dim
        else:
            self.embedding = None
            in_dim = 1

        layers: list[nn.Module] = [nn.Linear(in_dim, config.width), config.activation()]
        for _ in range(config.depth - 2):
            layers += [nn.Linear(config.width, config.width), config.activation()]
    ...
    def _ntk_mean_diag(
        ...
        The diagonal of each NTK block equals the mean squared gradient norm:
            mean(diag(K_ff)) = (1/N_f) Σ_i  ||∂R(t_i; θ)/∂θ||²
            mean(diag(K_uu)) = (1/N_u) Σ_j  ||∂z(t_j; θ)/∂θ||²

        Uses n_samples random points from t_col to keep cost tractable.
    ...
    def fit(
        ...
        *,
        ...
        ic_weight = tr_cfg.ic_weight    # may be updated adaptively
        ...
        for it in range(tr_cfg.n_iter + 1):
            ...
            # NTK-based adaptive weight update (after optimizer step)
            if (
    ...
    def compute_residuals(
        ...
        *,
        ...
        for frame in frames:
            ...
            if hard_ic:
                z = y0_t + t_in * self(t_in)   # t0 = 0 assumed
            else:
                z = self(t_in)
    ...
    def ntk(
        ...
        rhs: Callable,
        *,
        t_u: Optional[np.ndarray] = None,
        transform: Optional[Callable] = None,
        device=None,
    ) -> np.ndarray:
        """
        Empirical PINN NTK at the given collocation points.

        The PINN NTK governs gradient-descent training dynamics when the loss is
        the PDE residual.  Unlike the physics-blind base NTK, it incorporates the
        differential operator L through backpropagation:

            J_f[i, k] = ∂R(t_f_i; θ) / ∂θ_k
            R(t; θ)   = d/dt[ẑ(t; θ)] − rhs(t, ẑ(t; θ))
        """
        t_f = np.asarray(t_f, dtype=np.float32)
        N_f = len(t_f)
        ...
        t_f_tensor = torch.tensor(t_f, device=device).view(-1, 1)
        J_f = np.zeros((N_f, P), dtype=np.float32)

        self.eval()

        for i in range(N_f):
            # t_i must track gradients so autograd can compute dẑ/dt
            t_i = t_f_tensor[i:i+1].clone().requires_grad_(True)   # (1, 1)
            z_i = self(t_i)                                          # (1, n_vars)

            z_hat = transform(t_i, z_i) if transform is not None else z_i
            n_vars = z_hat.shape[1]

            # create_graph=True retains the graph so ∂R/∂θ can differentiate through dẑ/dt
            dz_hat_dt = torch.cat(
                [torch.autograd.grad(z_hat[:, j].sum(), t_i, create_graph=True)[0]
                 for j in range(n_vars)],
                dim=1,
            )

            R_i = dz_hat_dt - rhs(t_i, z_hat)
            grads = torch.autograd.grad(R_i.sum(), params, allow_unused=True)
            J_f[i] = np.concatenate([
                g.detach().cpu().numpy().ravel() if g is not None
        ...
        # Soft-IC block
        t_u_arr = np.asarray(t_u, dtype=np.float32)
        N_u = len(t_u_arr)
        t_u_tensor = torch.tensor(t_u_arr, device=device).view(-1, 1)
        J_u = np.zeros((N_u, P), dtype=np.float32)

        for i in range(N_u):
            t_i = t_u_tensor[i:i+1]
            z_i = self(t_i)
            z_hat = transform(t_i.detach(), z_i) if transform is not None else z_i
            grads = torch.autograd.grad(z_hat.sum(), params, allow_unused=True)
            J_u[i] = np.concatenate([
                g.detach().cpu().numpy().ravel() if g is not None
                else np.zeros(p.numel())
                for g, p in zip(grads, params)
            ])
    ...
    def ntk_spectrum(
        ...
        *,
        ...
        """Eigendecomposition of the PINN NTK; eigenvalues sorted descending."""
        K = self.ntk(t_f, rhs, t_u=t_u, transform=transform, device=device)
